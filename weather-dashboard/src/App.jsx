...
function buildProxyUrl(target, params) {
  ...
  return `/api/open-meteo/${target}${query ? `?${query}` : ""}`;
}
...
function App() {
  ...
  useEffect(() => {
    ...
    async function loadSelectedDate() {
      try {
        ...
        const weatherTarget = isPastDay ? "archive" : "forecast";
        ...
        const [weatherResponse, airResponse] = await Promise.all([
          fetchJson(
            buildProxyUrl(weatherTarget, weatherParams),
            weatherController.signal,
          ),
          fetchJson(
            buildProxyUrl("air-quality", airParams),
  ...
  async function handleSearch(event) {
    ...
    try {
      ...
      const data = await fetchJson(buildProxyUrl("geocoding", params));
